    ...
    protected static void communicateToUser(Source source, boolean useCcAccount, String userId, String userName, String subject, String[] messages, Locale locale) throws CommunicationException {
        ...
        if (Source.simulated.equals(source)) {
            ...
            try {
                ...
                // As the mail body contains mailto: links with the pre-formatted responses, user's direct responses should go to Support@ASE.com
                MailConnector.sendMailMessage(useCcAccount, true, userId, userName, subject, mailMessage.toString(), locale);
            }
            catch(Exception ex) {
