(function() { // To limit the scope of the private variables
     ...
     * @param {Boolean} showDebugMessage <code>true</code> if message conveyed with the 500 error code needs to be displayed
     */
    module.handleError = function(message, ioArgs, showDebugMessage) {
        ...
        if (ioArgs.xhr.status == 500) { // 500 == Server error
            ...
            if (showDebugMessage && ioArgs.xhr.responseText) {
                try {
                    var response = dojo.fromJson(ioArgs.xhr.responseText);
                    alert('Message: ' + response.exceptionMessage + '\nSource: ' + response.originalExceptionMessage);
                }
                catch(ex) {alert(ex)}
            }
            return;
